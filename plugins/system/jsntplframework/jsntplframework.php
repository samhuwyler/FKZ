...
class PlgSystemJSNTPLFramework extends JPlugin
{
	...
	public function onAfterInitialise ()
	{
		...
		if ($app->isAdmin())
		{
			...
			//Check if Advanced Templates Manager is installed, redirect to JSN TPL Framework
			$objJSNTplUtils	= JSNTplUtils::getInstance();
			$isInstalled 	= $objJSNTplUtils->checkAdvancedTemplatesManager();

			if ($app->input->getCmd('option') == 'com_advancedtemplates' && $id)
			{
				//Redirect from com_advancedtemplates to com_templates if template is made by JoomlaShine
				$templateInfo = JSNTplHelper::getTemplateInfo($id);

				if (JSNTplTemplateRecognization::detect($templateInfo->template))
				{
					return $app->redirect('index.php?option=com_templates&task=style.edit&id=' . $id);
				}
			}
			...
			if ($isInstalled)
			{
				$comAdvancedtemplates 			= JComponentHelper::getComponent('com_advancedtemplates', true);
				$comAdvancedtemplatesIsEnabled 	= $comAdvancedtemplates->enabled;

				if ($comAdvancedtemplatesIsEnabled)
				{
					if ($app->input->getCmd('option') == 'com_templates' && $app->input->getCmd('view') == 'styles')
					{
						return $app->redirect('index.php?option=com_advancedtemplates&task=styles');
					}
				}
			}
	...
	public function onExtensionAfterSave ($context, $data, $isNew)
	{
		...
		// duplicate megamenu for duplicated styles
		if ($task == 'duplicate' && $isNew && !$data->client_id && $context === 'com_templates.style')
		{
			$session 	= JFactory::getSession();
			$date 		= JFactory::getDate()->toSql();

			// if session JSNTPL_CLONE_STYLE_ID is not existed then created and assign value to it
			if ($session->has('JSNTPL_CLONE_STYLE_ID') == false)
			{
				$sessionData = $session->get('JSNTPL_CLONE_STYLE_ID', array());
				if (!count($sessionData))
				{
					$pks = $app->input->post->get('cid', array(), 'array');
					$session->set('JSNTPL_CLONE_STYLE_ID', $pks);
					$sessionData = $session->get('JSNTPL_CLONE_STYLE_ID', array());
				}
			}
			else
			{
				// if get session JSNTPL_CLONE_STYLE_ID value if it is existed
				$sessionData = $session->get('JSNTPL_CLONE_STYLE_ID', array());
			}

			// check if clone style is a of style of jsntpl
			if (!JSNTplTemplateRecognization::detect($data->template))
			{
				unset($sessionData[0]);
				$sessionData = array_values($sessionData);
				$session->set('JSNTPL_CLONE_STYLE_ID', $sessionData);
				$sessionData = $session->get('JSNTPL_CLONE_STYLE_ID', array());

				if (!count($sessionData))
				{
					$session->clear('JSNTPL_CLONE_STYLE_ID');
				}

				return;
			}

			$megamenuItems = JSNTplHelper::getMegamenuItemListByStyleID($sessionData[0]);

			if (count($megamenuItems))
			{
				foreach ($megamenuItems as $megamenuItem)
				{
					$query = $dbo->getQuery(true);
					$columns = array('style_id', 'language_code', 'menu_type', 'created', 'modified', 'params');
					$values = array(intval($data->id), $dbo->quote($megamenuItem->language_code),
							$dbo->quote($megamenuItem->menu_type), $dbo->quote($date),
							$dbo->quote($date), $dbo->quote($megamenuItem->params));

					$query
					->insert($dbo->quoteName('#__jsn_tplframework_megamenu'))
		...
		elseif ($task == 'save2copy' && $isNew && !$data->client_id && $context === 'com_templates.style')
		{
			...
			if ($styleID)
			{
				...
				$megamenuItems = JSNTplHelper::getMegamenuItemListByStyleID($styleID);
